...
- **Floating Add action:** the persistent bottom-right FAB represents the app’s primary constructive action. Use the hybrid plus-icon and object label (`Expense`) so icon and text are not redundant; retain floating elevation and keep secondary actions in the header.
...
- **Category detail sheet:** compose the shared bottom-sheet shell, identity header, three-value financial summary, semantic progress indicator, compact labeled contextual actions, a month-filter trigger, and shared transaction rows. Currency is owned by `Money` and must never be repeated by a surrounding stat component.
- **Period filter:** use one compact calendar-trigger control displaying the active period and open the platform picker for non-linear selection. Previous/next steppers belong on primary time-series screens where sequential browsing is the core task; do not spend an entire section-header row on them inside a detail sheet.
- **Date picker:** every day- and month-granularity field uses the shared `DatePicker`/`MonthPicker` trigger: calendar icon, concise localized value, trailing chevron, 44px minimum target, and the standard control border/radius. The picker body may reflect its granularity, but its trigger must not change between forms, planning, and detail filters.
...
- **Calendar picker:** show one compact month grid with previous/next navigation. Do not duplicate `Today` or `Yesterday` shortcuts when those dates are already visible and selectable in the grid. Mark the actual current date independently from the selected date using the existing green status-badge color pair; if today is selected, use the deeper green selected treatment.
...
- Focus indicators are at least 3px and remain visible above fixed navigation.
...
- **Insights:** month control, compact analytical cards, accessible chart alternatives, and transaction history. A failed request becomes an actionable error state, never a permanent skeleton.
- **Accounts and Settings:** use the same title, spacing, control, and surface primitives as the app shell and sheets.
